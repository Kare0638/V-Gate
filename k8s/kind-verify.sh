...
cleanup() {
    ...
    sleep 1  # let tee flush
    {
        ...
        echo "- **Step 13** — \`state_changes_total\` separates three things."
        echo "  \`admitted\` is a discovered worker entering rotation for the"
        echo "  first time, after a probe proved it usable; discovered arrivals"
        echo "  start out of rotation, because the worker Service publishes"
        echo "  not-ready addresses on purpose and a pod still loading weights"
        echo "  is therefore in the DNS answer. \`removed\` here is normal"
