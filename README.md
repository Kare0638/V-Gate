...
**Multi-worker serving is measured on real GPUs: 1.99x across two A100s** ([scaling_a100_2gpu.md](benchmarks/results/scaling_a100_2gpu.md)) — 5.4 to 10.8 req/s on `Qwen2.5-32B` with one A100-SXM4-80GB per worker, and p95 halved. The synthetic-backend run scales the same way to four workers ([scaling.md](benchmarks/results/scaling.md)). Inference happens in separate `role: worker` processes whose membership the gateway discovers from DNS; it routes across them round-robin, probes their health in the background, removes failing workers from rotation, and lets them rejoin once they recover. A killed worker does not fail requests — traffic shifts to the survivors, and `503` with `Retry-After` is returned only when every worker is down.
...
| **1-vs-N worker scaling (synthetic)** | [2.00x at 2 workers, 3.89x at 4](benchmarks/results/scaling.md), 96–99% of ideal, 3 repeats per point, 0 failures across all 18 runs | Dry-run workers with declared capacity: measures how well one gateway feeds N backends. Extends the real-GPU result above to worker counts a rented pair cannot reach |
...
| **2x A100 worker scaling (real GPUs)** | [`scaling_a100_2gpu.md`](benchmarks/results/scaling_a100_2gpu.md): **1.99x at two workers**, 5.4 -> 10.8 req/s, p95 28.3s -> 14.5s, 192/192 request split, 0 failures, 3 repeats | `Qwen2.5-32B-Instruct` FP16, one A100-SXM4-80GB per worker, NVLink host. Real generation, not the dry-run backend |
...
- [x] Make gateway readiness reflect whether it can serve — `/ready` gates startup on having had a usable worker, `/health` stays on liveness; readiness latches on purpose, because failing it on pool loss would empty the Service and downgrade a retryable `503` to a connection refused
