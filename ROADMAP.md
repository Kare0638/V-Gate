...
The target is a distributed LLM inference system: a gateway tier that owns admission, batching, caching, and routing, in front of a pool of inference workers. Both tiers exist today, and the pool is measured to help on real hardware: 1.99x across two A100s ([scaling_a100_2gpu.md](benchmarks/results/scaling_a100_2gpu.md)), extending to 3.89x at four workers with the synthetic backend ([scaling.md](benchmarks/results/scaling.md)). What does not exist is load-aware routing to make it help more.
...
- Multi-GPU is now measured: two A100-SXM4-80GB, one per worker, `Qwen2.5-32B` FP16, 1.99x throughput and halved p95 ([scaling_a100_2gpu.md](benchmarks/results/scaling_a100_2gpu.md)). Tensor parallelism is still not measured -- the run used data parallelism across workers, which is what this project contributes; sharding a model across devices is the engine's job and says little about the gateway.
- The gateway's outbound thread pool was the binding constraint on real hardware, and the A100 run is what exposed it. `RemoteBackend.generate` is a synchronous httpx call through `run_in_executor`, so outbound concurrency is capped by the default pool of `min(32, cpu_count + 4)`. With 100ms synthetic generations that read as a ~180 req/s ceiling and looked like headroom; with 6-second real generations it is 32/6 = 5.3 req/s, and no number of GPUs moves it. The pool is now sized to the admission limit, which restored linear scaling. That is a bound, not a fix: threads blocked on HTTP are pure overhead, and an async client would take them out of the path.
- Tensor parallelism, KV-cache pressure at realistic context lengths, and scheduling when a model does not fit on one device remain untested. The A100 run measured data parallelism across workers, not sharding within one; `benchmarks/bench_tensor_parallel.py` exists for the sharding question and has only been run at TP=1.
...
- Readiness is a startup gate, not a continuous dependency check, and that is a deliberate narrowing of what this document previously specified. `/ready` is false until the gateway has had a usable worker at least once, which closes the cold start where Kubernetes routed to a gateway holding zero workers; `/health` stays on liveness, since a process that is up but has no pool must not be restarted for it. The earlier plan here — "a `/ready` that fails when no worker is in rotation" — was wrong: every replica shares one worker pool, so failing readiness on pool loss removes all of them from the Service simultaneously, and clients get a connection refused instead of `503` with `Retry-After`. Less information, and no replica is better placed to take over.
...
8. [x] Split readiness from liveness on the gateway. `/ready` reports whether
   this process has become able to serve and backs the startup and readiness
   probes; `/health` keeps answering "the process is alive" and backs liveness
   and the gateway's own worker probing. Readiness latches once satisfied --
   see the positioning note above for why a continuous check on the pool would
   make a partial outage into a total one.
